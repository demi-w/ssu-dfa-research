...
fn ruleset_parsing() {
    ...
    let solve_test = build_default1dpeg::<MinkidSolver>().unwrap();
    assert!(
        solve_test.get_ruleset() == &Ruleset::from_string(&solve_test.get_ruleset().to_string()),
        "default1dpeg ruleset failed to recreate itself"
    );

    let solve_test = build_threerule1dpeg::<MinkidSolver>().unwrap();
    assert!(
        solve_test.get_ruleset() == &Ruleset::from_string(&solve_test.get_ruleset().to_string()),
        "threerule1dpeg ruleset failed to recreate itself"
    );

    let solve_test = build_defaultsolver::<MinkidSolver>().unwrap();
    assert!(
        solve_test.get_ruleset() == &Ruleset::from_string(&solve_test.get_ruleset().to_string()),
        "defaultsolver ruleset failed to recreate itself"
    );

    let solve_test = build_threerulesolver::<MinkidSolver>().unwrap();
    assert!(
        solve_test.get_ruleset() == &Ruleset::from_string(&solve_test.get_ruleset().to_string()),
        "threerulesolver ruleset failed to recreate itself"
    );
}

fn assert_k<S>(solver: &S, k: usize, test_string: &str)
where
    S: SRSSolver,
{
    ...
    for bad_k in 1..k {
        ...
        assert!(
            (bad_dfa >= final_dfa) as usize >= is_superset as usize,
            "Incorrectly deemed superset for {} when k = {}",
            test_string,
            bad_k
        );
        ...
        assert!(
            bad_dfa != final_dfa,
            "Incorrect k for {}, {} works fine",
            test_string,
            bad_k
        );
        assert!(
            !is_correct,
            "Incorrectly deemed correct for {} when k = {}",
            test_string, bad_k
        );
    }
    assert!(
        solver.is_correct(&final_dfa),
        "Incorrectly deemed incorrect for {}",
        test_string
    );
}
