...
use enum_iterator::{cardinality,Sequence, all, All};
...
fn worker_thread(translator : Arc<SRSTranslator>, sig_set : Vec<Vec<SymbolIdx>>, input : Arc<SegQueue<(Vec<SymbolIdx>,usize)>>, output : Arc<SegQueue<(Vec<bool>,usize)>>) -> thread::JoinHandle<()> {
    thread::spawn(move || {
        while true {
            match input.pop() {
                Some(input_string) => 
                {
                    ...
                    let result = translator.sig_with_set_batch(&input_string.0, &sig_set);
...
impl SRSTranslator {
    ...
    */
    ...
    fn sig_with_set_batch(&self, board : &Vec<SymbolIdx>, sig_set : &Vec<Vec<SymbolIdx>>) -> Vec<bool> {
        
        let mut result : Vec<bool> = Vec::new();
        for sig_element in sig_set {
            let mut new_board = board.clone();
            new_board.extend(sig_element);
            result.push(self.bfs_solver_batch(&new_board));
        }
        result
    ...
    fn board_to_next_batch(&self,board : &Vec<SymbolIdx>, sig_set : &Vec<Vec<SymbolIdx>>,input : Arc<SegQueue<(Vec<usize>,usize)>>, output : Arc<SegQueue<(Vec<bool>,usize)>>) -> Vec<(Vec<bool>,Vec<SymbolIdx>)> {
        let mut results : Vec<(Vec<bool>,Vec<SymbolIdx>)> = Vec::new();
        for sym in 0..self.symbol_set.length {
            let mut new_board = board.clone();
            new_board.push(sym);
            input.push((new_board.clone(),sym));
            results.push((vec![],new_board));

        }
        let mut results_recieved = 0;
        while results_recieved < self.symbol_set.length {
            match output.pop() {
                Some(output_result) => {
                    results[output_result.1].0 = output_result.0; 
                    results_recieved+=1;
                },
                None => {std::thread::sleep(time::Duration::from_millis(10));}
            }
    ...
    fn dfa_with_sig_set_batch(&self, sig_set : &Vec<Vec<SymbolIdx>>) -> DFA {
        ...
        let output : Arc<SegQueue<(Vec<bool>,usize)>> = Arc::new(SegQueue::new());

        for _ in 0..6 {
            worker_thread(thread_translator.clone(),sig_set.clone(), input.clone(), output.clone());
        }
        ...
        let start_accepting = self.sig_with_set_batch(&vec![],&sig_set);
        ...
        while new_boards.len() > 0 {
            ...
            for (start_idx,board) in &old_boards {
                ...
                let next_results = self.board_to_next_batch(&board, sig_set, input.clone(), output.clone());
...
fn verify_to_len(&mut self,test_dfa : DFA, n:usize){
    let total_boards = (self.symbol_set.length as u64).pow(n as u32 + 1) -1;
