...
pub fn build_2dpeg_goal() -> DFA {
    ...
    let mut trans_table = vec![
        vec![1, 2, 2 + 16, 2 + 16 * 2, 2 + 16 * 2, 10, 2, 10],
        vec![1, 3, 3 + 16, 3 + 16 * 2, 3 + 16 * 2, 10, 3, 10],
    ];
    for point in 0..=2 {
...
pub fn build_2xnswap_rs() -> Ruleset {
    ...
    for i in 0..(8 as SymbolIdx) {
        ...
        let sml = i % 2;
        rules.push((
            vec![1 + big, 1 + mid, 0 + sml],
            vec![0 + big, 0 + mid, 1 + sml],
        ));
        rules.push((
            vec![0 + big, 1 + mid, 1 + sml],
            vec![1 + big, 0 + mid, 0 + sml],
        ));
    }

    Ruleset::from_vec(rules, symbol_set.clone())
}
...
pub fn build_flip_rs() -> Ruleset {
    ...
    for i in 0..8 {
        rules_vec.push((
            vec![i / 4 % 2, i / 2 % 2, i % 2],
            vec![1 - i / 4 % 2, 1 - i / 2 % 2, 1 - i % 2],
        ))
    }
    Ruleset::from_vec(rules_vec, b_symbol_set.clone())
}
    ...
    for rule in rules_vec {
        ...
        for i in 0..(rule.0.len() - k + 1) {
            ...
            for (rule_sym_idx, rule_sym) in rule.0.iter().enumerate() {
                start_sym_idx += rule_sym
                    * (b_symbol_set.length as SymbolIdx)
                        .pow((rule.0.len() - rule_sym_idx - 1) as u32);
            }
            ...
            for (rule_sym_idx, rule_sym) in rule.1.iter().enumerate() {
                end_sym_idx += rule_sym
                    * (b_symbol_set.length as SymbolIdx)
                        .pow((rule.1.len() - rule_sym_idx - 1) as u32);
            }
...
pub fn build_threerulesolver<S>() -> Result<S, DomainError>
where
    S: SRSSolver,
{
...
pub fn build_defaultsolver<S>() -> Result<S, DomainError>
where
    S: SRSSolver,
{
...
pub fn build_2xnswap<S>() -> Result<S, DomainError>
where
    S: SRSSolver,
{
    let symbol_set = SymbolSet {
...
pub fn build_default1dpeg<S>() -> Result<S, DomainError>
where
    S: SRSSolver,
{
...
pub fn build_threerule1dpeg<S>() -> Result<S, DomainError>
where
    S: SRSSolver,
{
...
pub fn build_flip<S>() -> Result<S, DomainError>
where
    S: SRSSolver,
{
...
pub fn build_flipx3<S>() -> Result<S, DomainError>
where
    S: SRSSolver,
{
...
pub fn build_default2dpegx3<S>() -> Result<S, DomainError>
where
    S: SRSSolver,
{
