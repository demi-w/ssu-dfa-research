...
fn worker_thread(translator : Arc<SRSTranslator>, input : Arc<SegQueue<(Vec<usize>,usize)>>, output : Arc<SegQueue<(bool,usize)>>) -> thread::JoinHandle<()> {
    thread::spawn(move || {
        while true {
            match input.pop() {
                Some(input_string) => 
                {
                    if input_string.1 == usize::MAX && input_string.0 == vec![69,420] {
                        return;
                    }
...
impl SRSTranslator {
    ...
    */
    ...
    fn sig_with_set_batch_request(&self, board : &Vec<SymbolIdx>, sig_set : &Vec<Vec<SymbolIdx>>, input : Arc<SegQueue<(Vec<usize>,usize)>>, output : Arc<SegQueue<(bool,usize)>>, start_index : usize) {
        for sig_element in sig_set.iter().enumerate() {
            let mut new_board = board.clone();
            new_board.extend(sig_element.1);
            input.push((new_board,sig_element.0+start_index));
        }
        /*
    ...
    fn sig_with_set_batch_grab(&self, answer_vec : &BitVec, sig_set_len : usize, start_index : usize) -> Vec<bool> {
        let mut result : Vec<bool> = vec![false;sig_set_len];
        for i in 0..sig_set_len {
            result[i] = answer_vec[i+start_index];
        }
    ...
    fn board_to_next_batch_request(&self,board : &Vec<SymbolIdx>, sig_set : &Vec<Vec<SymbolIdx>>,input : &Arc<SegQueue<(Vec<usize>,usize)>>, output : &Arc<SegQueue<(bool,usize)>>, start_index : usize) {
        let mut cur_index = start_index;
        for sym in 0..self.symbol_set.length {
            let mut new_board = board.clone();
            new_board.push(sym);
            self.sig_with_set_batch_request(&new_board,sig_set,input.clone(),output.clone(),cur_index);
            cur_index += sig_set.len();
        }
    }
    fn board_to_next_batch_grab(&self,board : &Vec<SymbolIdx>,answer_vec : &BitVec, sig_set_len : usize, start_index : usize) -> Vec<(Vec<bool>,Vec<SymbolIdx>)> {
        let mut results : Vec<(Vec<bool>,Vec<SymbolIdx>)> = Vec::new();
        let mut cur_index = start_index;
        for sym in 0..self.symbol_set.length {
            let mut new_board = board.clone();
            new_board.push(sym);
            results.push((self.sig_with_set_batch_grab(answer_vec,sig_set_len,cur_index),new_board));
            cur_index += sig_set_len;
        }
        results
    }
    ...
    fn dfa_with_sig_set_batch(&mut self, sig_set : &Vec<Vec<SymbolIdx>>) -> DFA {
        ...
        //Function signature only mutable to lazily support this call
        ...
        while new_boards.len() > 0 {
            ...
            for (start_idx,board) in &old_boards {
                ...
                self.board_to_next_batch_request(&board, sig_set, &input, &output, cur_idx);
                cur_idx += sig_set.len() * self.symbol_set.length;
            }
            let mut answer_vec = bitvec![0;cur_idx];
            let mut num_recieved = 0;
            while num_recieved < cur_idx {
                match output.pop() {
                    Some((value,idx)) => {answer_vec.insert(idx,value); num_recieved+=1},
                    None => {std::thread::sleep(time::Duration::from_millis(100));println!("{}",num_recieved)}
                }
            }
            println!("done");
            cur_idx = 0;
            for (start_idx,board) in &old_boards{
                let next_results = self.board_to_next_batch_grab(board,&answer_vec, sig_set.len(),cur_idx);
                cur_idx += sig_set.len() * self.symbol_set.length;
    ...
    fn create_workers(&self, worker_count : usize) -> (Arc<SegQueue<(Vec<usize>,usize)>>,Arc<SegQueue<(bool,usize)>>) {
        let thread_translator : Arc<SRSTranslator> = Arc::new(self.clone());

        let input : Arc<SegQueue<(Vec<usize>,usize)>> = Arc::new(SegQueue::new());
    ...
    fn terminate_workers(&self, input : Arc<SegQueue<(Vec<usize>,usize)>>, worker_count : usize) {
        for _ in 0..worker_count {
            input.push((vec![69, 420], usize::MAX))
        }
    }
    ...
    for _ in 0..n {
        ...
        for (idx,i) in signature_set_old.iter().enumerate() {
            for symbol in 0..self.symbol_set.length {
                ...
                input.push((test_board.clone(),idx*self.symbol_set.length + symbol));
            }
        }
        let mut num_recieved = 0;
...
impl DFA {
    fn contains(&self, input : &Vec<SymbolIdx>) -> bool {
        ...
        for i in input {
            state = self.state_transitions[state][*i];
        }
...
fn main() {
    ...
    let possible_dfa = translator.dfa_with_sig_set_batch(&translator.build_sig_k(5));
    ...
    for rule in ruleset.rules {
        ...
        for i in 0..(rule.0.len() - k+1) {
            ...
            for (rule_sym_idx, rule_sym) in rule.0.iter().enumerate() {
                start_sym_idx += rule_sym*b_symbol_set.length.pow((rule.0.len()-rule_sym_idx-1) as u32);
            }
            start_sym_idx *= b_symbol_set.length.pow((i) as u32);

            let mut end_sym_idx = 0;
            for (rule_sym_idx, rule_sym) in rule.1.iter().enumerate() {
                end_sym_idx += rule_sym*b_symbol_set.length.pow((rule.1.len()-rule_sym_idx-1) as u32);
            }
            end_sym_idx *= b_symbol_set.length.pow(i as u32);
        ...
        for i in 0..k {
            ...
            for j in 0..k {
                ...
                let pow_num = b_symbol_set.length.pow(j as u32);
                if i == j {
    ...
    let possible_dfa = translator.dfa_with_sig_set_batch(&translator.build_sig_k(5));
    possible_dfa.save("threerule2dpegx3"); 
    //println!("{:?}",new_rules);
    */
