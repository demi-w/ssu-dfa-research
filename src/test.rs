...
pub fn test_standard_examples<S: SRSSolver>() {
    ...
    assert!(
        solve_test.is_correct(&solve_test.run(5, vec![])),
        "1dpeg failed for {}",
        type_name::<S>()
    );
    ...
    assert!(
        solve_test.is_correct(&solve_test.run(5, vec![])),
        "threerule1dpeg failed for {}",
        type_name::<S>()
    );

    let solve_test = build_defaultsolver::<S>().unwrap();
    assert!(
        solve_test.is_correct(&solve_test.run(6, vec![])),
        "defaultsolver failed for {}",
        type_name::<S>()
    );
