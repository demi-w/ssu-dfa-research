...
use bitvec::prelude::*;
use petgraph::{algo::toposort, graph::NodeIndex, prelude::DiGraph, Direction};
...
impl Solver for SubsetSolver {
    ...
    fn run_internal(
        ...
        self.trans_table
            .push((1..=self.rules.symbol_set.length).collect());
        ...
        while last_finished < last_known {
            if is_debug {
                dfa_events
                    .send((
                        DFAStructure::Dense(self.trans_table.clone()),
                        SSStructure::BooleanMap(self.unique_sigs.clone()),
                    ))
                    .unwrap();
            }
            ...
            self.sig_sets
                .resize(self.sig_sets.len() + new_states, bitvec![0;sig_set.len()]);
                ...
                while let Some(nx) = explore.pop() {
                    for neighbor in link_graph.neighbors_directed(nx, Direction::Incoming) {
                        if link_graph[neighbor][0] >= last_known && !visit.contains(&neighbor) {
                            ...
                            self.solved_yet[link_graph[neighbor][0] - last_known] |=
                                self.sig_sets[origin].clone();
            ...
            for node in toposort(&reverse_link_graph, None).unwrap() {
                if link_graph[node][0] >= last_known {
                    ...
                    let connecting_state =
                        (link_graph[node][0] - last_known) / self.rules.symbol_set.length;
                    let connecting_symbol = ((link_graph[node][0] - last_known)
                        % self.rules.symbol_set.length)
                        as SymbolIdx;
            ...
            for pros_state in link_graph.node_indices() {
                ...
                let connector = match link_graph[pros_state].iter().find(|&x| x < &last_known) {
                    ...
                    None => {
                        match self
                            .unique_sigs
                            .get(&self.sig_sets[link_graph[pros_state][0]])
                        {
                            Some(i) => *i,
                            None => {
                                let connecting_state = (link_graph[pros_state][0] - last_known)
                                    / self.rules.symbol_set.length
                                    + last_finished;
                                let connecting_symbol = ((link_graph[pros_state][0] - last_known)
                                    % self.rules.symbol_set.length)
                                    as SymbolIdx;
                                let mut new_board =
                                    recent_strings[connecting_state - last_finished].clone();
                                new_board.push(connecting_symbol);
                                self.unique_sigs.insert(
                                    self.sig_sets[link_graph[pros_state][0]].clone(),
                                    new_known + last_known,
                                );
                                new_known += 1;
                ...
                for dupe in &link_graph[pros_state] {
                    ...
                    let connecting_state =
                        (dupe - last_known) / self.rules.symbol_set.length + last_finished;
            ...
            for i in 0..new_known {
                self.trans_table.push(
                    ((last_known + new_known + i * self.rules.symbol_set.length)
                        ..=(last_known + new_known + (i + 1) * self.rules.symbol_set.length - 1))
                        .collect(),
                )
            }
...
impl SubsetSolver {
    fn bfs_solver_sub(
        ...
        state_idx: usize,
        sig_idx: usize,
        investigated: &mut HashSet<(usize, usize)>,
    ) -> bool {
        /*if !investigated.insert((state_idx,sig_idx)) {
            return false;
        }*/

        if state_idx < self.trans_table.len()
            || self.solved_yet[state_idx - self.trans_table.len()][sig_idx]
        {
        ...
        for new_board in self.single_rule_hash(&start_board) {
            ...
            if self.bfs_solver_sub(
                &new_board,
                dfa_idx,
                self.rules
                    .symbol_set
                    .find_in_sig_set(new_board[board_idx..].iter()),
                investigated,
            ) {
                self.sig_sets[state_idx].set(sig_idx, true);
                break;
