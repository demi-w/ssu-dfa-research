...
pub trait SRSSolver where Self : Solver<Vec<SymbolIdx>,String,bool> + Sized{
    ...
    fn get_mutator(&self) -> impl Fn(Vec<SymbolIdx>,SymbolIdx) -> Vec<SymbolIdx> {
        |mut state, input| {state.push(input); state}
    }
    const MUTATOR : fn(&Self, Vec<SymbolIdx>,SymbolIdx) -> Vec<SymbolIdx> = |&_, mut state, input| {state.push(input); state};
    ...
    fn single_rule_hash(&self, start_board : &Vec<SymbolIdx>) -> Vec<Vec<SymbolIdx>> {
        ...
        for lftmst_idx in 0..start_board.len() {
            for slice_length in self.get_ruleset().min_input..core::cmp::min(self.get_ruleset().max_input,start_board.len()-lftmst_idx)+1 {
