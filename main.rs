...
/* 
tortured pretty-printing attempt.
I guess I should just stick to debugging when under the influence of alcohol??
lmao
struct TimeSegmentPrinting {
    last_time : time::Instant,
    iteration_time : time::Duration,
    cur_string : String,
    newest_segment : String
}
impl TimeSegmentPrinting {
    fn new() -> TimeSegmentPrinting {
        TimeSegmentPrinting {
            last_time : time::Instant::now(),
            iteration_time : time::Duration::from_secs(0),
            cur_string : "".to_owned(),
            newest_segment : "".to_string()
        }
    }

    fn new_iteration(&self) {
        println!("{}",self.cur_string);
        self.cur_string = "".to_owned();
        self.newest_segment = "".to_owned();
    }

    fn new_segment(&self, name : &str) {
        if self.newest_segment != "" {
            self.cur_string.push_str(" | ");
        }
        self.cur_string.push_str(&self.newest_segment);
        self.cur_string.push_str(&format!(": {}ms", (time::Instant::now() - self.last_time).as_millis()));
        self.newest_segment = name.to_string();
        print!("{}\r", self.cur_string);
...
impl SRSTranslator {
    ...
    fn add_link(&self, mut link_graph : &mut DiGraph<(),(Vec<SymbolIdx>,Vec<SymbolIdx>)>, lhs : NodeIndex, rhs : NodeIndex, lhs_obligation : &[SymbolIdx], rhs_obligation : &[SymbolIdx]) -> (bool,bool) {
...
fn dfa_with_sig_set_minkid(&mut self, sig_set_size : usize) -> DFA {
    ...
    let bad_state_idx = 60;
    let bad_minkids = vec![355, 349, 317, 318, 275, 202, 226, 203, 201, 231, 89];
    let bad_iteration = 5;
        ...
        for prospective_idx in *iteration_lens.last().unwrap()..dfa_graph.node_count() {
            ...
            for edge in link_graph.edges_directed(prospective_node, Outgoing) {
                ...
                if self.partial_link(&mut dfa_graph, sig_set, edge.weight(), edge.source(), edge.target()) {
                    ...
                    if !has_bad && bad_iteration == iteration_lens.len()-2 && bad_state_idx == prospective_idx && bad_minkids.iter().any(|x| dfa_graph[prospective_node].minkids.contains(&NodeIndex::new(*x))) {
                        println!("The bad has occurred. RHS is {}, weight is {:?}", edge.target().index(), edge.weight());
                        has_bad = true;
                    }
                }
            }
...
fn main() {
    ...
    assert!(translator.dfa_with_sig_set_subset(12) == DFA::load("2xnswap").unwrap(), "2xnswap failed");
} 
